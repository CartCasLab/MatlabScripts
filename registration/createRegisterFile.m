function createRegisterFile(registerFileName, fixedImage, movingImage, Tout, ImageOut, varargin)
...
% - varargin = tolerance, optimization and xform_in can be additionally defined
%OUT: 
% command file for a rigid registration with plastimatch register

%% modify with parsing !!!!!!!!!!!1
...
% if nargin > 7 && strcmp(varargin{1}, 'tol')
%     toll = varargin{2}; %improve robustness
% elseif nargin==6 && strcmp( regType(1:2), 'T1' )
%     toll = 0.000001;
% else
%     toll = 0.00001; 
% end
% if nargin > 7 && strcmp(varargin{3}, 'opt')
%     optimi = varargin{4}; %improve robustness
%     grad_tol = 'grad_tol';
% else
%     optimi = 'versor'; %Stop optimization if change in score between it falls below this value
%     grad_tol = 'rsg_grad_tol';

function createRegisterFile_CTonADC_affine(registerFileName, fixedImage, movingImage, Tout, ImageOut, varargin)
...
[~, movType_i] = strtok( fliplr(movingImage(1:end-4)), '_');
if ~isempty(movType_i)
    movType = fliplr(movType_i);
else
    [movType_i, ~] = strtok( fliplr(movingImage(1:end-4)), filesep);
    movType = fliplr(movType_i);
end
%default values
toll = 0.000001;
...
elseif sum( fixType == movType ) %|| ...%for CT-MRI do not use translation (MSE-based)
    fprintf( fid, ['\n[STAGE]\n' 'xform=translation\n\n'] );
else
    fprintf( fid, ['\n[STAGE]\n' 'xform=align_center\n\n'] );
end

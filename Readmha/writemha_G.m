function writemha_G(fn,A,offset,spacing,anatom_orient,transf_m,type)
% writemha_G(fn,A,offset,spacing,anatom_orient,transf_m,type)
%   fn        filename
%   A         Volume
%   offset
%   spacing
%   anatom_orient String with anatomical orientation ('LPI', 'RAI', ecc)
%   transf_matrix Transformation matrix in vector form (ex. [1 0 0 0 1 0 0
%   0 1] )
...
fprintf (fp,'\n');
fprintf (fp,'AnatomicalOrientation = %s\n',anatom_orient);
if (ndims(A) == 4)
  fprintf(fp,'ElementNumberOfChannels = 3\n');
end
fprintf (fp,'TransformMatrix = ');
fprintf (fp,' %g',transf_m');
fprintf (fp,'\n');
